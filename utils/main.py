    ...
    if enable_reporting:
        ...
        def task_abuseipdb():
            ...
            if ABUSEIPDB_API_KEY:
                lines.append("\n[+] AbuseIPDB Reporting:")
                local_reported_ips = set()
                
                if sender_ip and sender_ip not in local_reported_ips:
                    ...
                    lines.append(f"    Sender IP {sender_ip}: {status}")
                    local_reported_ips.add(sender_ip)
                ...
                for ip in report_ips:
                    if ip not in local_reported_ips:
                        success = report_to_abuseipdb(
                            ip=ip,
                            categories=[3, 7, 11, 15, 17, 19, 20],
                        ...
                        status = "✅ Success" if success else "❌ Failed"
                        lines.append(f"    URL Hosting IP {ip}: {status}")
                        local_reported_ips.add(ip)
            else:
            ...
            if vt_configured:
                ...
                if urls_to_report:
                    ...
                    for base_url, children in domain_map.items():
                        ...
                        status = "✅ Success" if success else "❌ Failed"
                        lines.append(f"    Base URL {base_url}: {status}")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=9) as executor:
            ...
            for future in concurrent.futures.as_completed(future_map):
                ...
                try:
                    ...
                    logger.error(f"Reporting task {idx} failed: {e}")
            
            for res in results:
                report_lines.extend(res)
